...
app.get('/rankdata_m', async (req, res) => {
  try {
    ...
    // b_user 테이블에서 데이터 가져오기
    const rankdb = await connection.query('SELECT Nickname, BScore, LScore, Class FROM m_user WHERE Nickname != "admin_m" ORDER BY (BScore + LScore) DESC');
    const winlose = await connection.query('SELECT winner, win2, win3, win4, loser, lose2, lose3, lose4 FROM m_record');
...
app.get('/recorddata_m', async (req, res) => {
  try {
    ...
    // b_record 테이블에서 데이터 가져오기
    const allrecord = await connection.query('SELECT OrderNum, Date, Winner, win2, win3, win4, loser, lose2, lose3, lose4, wscore, lscore FROM m_record');
    connection.release();
...
// 서버 레코드 상대전적 데이터 가져오는 엔드포인트
app.get('/opprecord_m', async (req, res) => {
  try {
    // 기존에 생성한 전역 풀을 사용
    const connection = await pool.getConnection();
